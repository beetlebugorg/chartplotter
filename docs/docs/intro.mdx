...
- **Builds to one self-contained binary.** The S-101 catalogue and the web
  frontend are built into the program, so the binary you build runs from a
  single file. (You build it yourself — the project publishes no binaries; see
