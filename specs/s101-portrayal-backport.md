...
2. ~~**Colours** (A)~~ — **DONE** (exact match, `cmd/s101-color-diff`; loader `catalog.LoadColorProfile` → `ColorProfile` day/dusk/night). Remaining: point `assets/colortables.go` at it (currently still DAI-backed).
3. ~~**SVG symbols** (B) — rasterizer de-risk + production atlas~~ — **DONE**. `pkg/s100/symbols` is the reusable flattener+rasterizer (CSS resolution, viewBox/stroke workarounds, RGBA→straight-alpha NRGBA). `assets.SpriteAtlasS101(symbolsDir, cssPath)` packs all 724 symbols into the *same* atlas format the S-52 path emits (512-px-wide PNG + sprites.json with pivots) — drop-in for the client. Verified: builds 512×4823, BCNCAR01 pivot (17.3,25.7) = −minX·8/−minY·8 as expected; the S-52 atlas test still passes. Remaining: dusk/night palette atlases (or runtime tint); wire into the asset emit step.
...
7. ~~**Bridge** (E) — S-57→S-101 features/attributes~~ — **DONE for simple attributes**. `pkg/s100/fc` derives the name maps from `<alias>`; `internal/engine/s101` (`Engine`+`host.go`) runs the real framework with `HostGet*` type introspection backed by the Feature Catalogue and an S-57→S-101 feature/attribute adapter, then dispatches each feature's rule. **Proven end-to-end**: S-57 `SILTNK`+`{CATSIL:3,CONVIS:1}` → `SiloTank`+`{categoryOfSiloTank:3,visualProminence:1}` → rule emits `PointInstruction:TOWERS03`; `CATSIL=2`→`TNKCON02`; surface→`ColorFill:CHBRN`; unmapped class→`UNMAPPED:` marker. Remaining: complex attributes + associations, and the full `PortrayalMain` wrapper (feature-name text, nautical info, dates).
8. **Wire Lua as the lookup+CSP stage** — instruction-stream → primitives; placeholders live. **Cutover seam DONE**: `portrayal.S101Builder.Build(*s57.Feature) → FeatureBuild` is the drop-in replacement for `BuildFeature` — runs the fc-backed engine, parses the stream, lowers each draw onto the feature's real geometry (point/line/area), unmapped class → `QUESMRK1`. Tested end-to-end with real `s57.Feature`s (point→`SymbolCall TOWERS03` at anchor; polygon→`FillPolygon CHBRN` on rings). Geometry plumbing complete. Added `s57.NewFeature` constructor. Remaining: replace the `BuildFeature`/`BuildFeaturePasses` call sites in `bake/` with this; multi-pass (plain/symbolized, simplified/paper) + text labels via the full `PortrayalMain` wrapper.
8b. ~~**`fs.FS` refactor** (embed precondition)~~ — **DONE**. `catalog.LoadFS`, `fc.LoadBytes`, `s101.NewEngineFS`, `assets.SpriteAtlasS101FS` — same code serves `embed.FS` (production) and `os.DirFS` (tests).
9. **Baker surgery** (replace-then-delete; **decided: embed catalogue, no `--portrayal` flag**):
   - a/b. ~~Portrayal seam~~ — **DONE**. `bake.Baker.portrayer` (internal seam, `SetPortrayer`) + `bake.NewS101Portrayer` (wraps `portrayal.NewS101Builder` = engine + fc + drawing catalogue); `bake.go` `AddCell` uses it instead of `BuildFeaturePasses` when set. **Verified: a real NOAA cell (US4MD81M) bakes to 1033 tiles through S-101.** S-52 path unchanged (nil portrayer), tests green.
   - c. Asset emit — **mostly DONE**. `assets.EmitS101`/`EmitS101FS`: colortables.json (from `colorProfile.xml`, byte-identical to S-52), `linestyles.json` (from `catalog` LineStyles; reuses the S-52 dash analysis; composite→primary component), and sprite.{png,json} (724 S-101 symbols). Wired into the CLI: `chartplotter emit-assets --s101 <PortrayalCatalog> <out>`. Remaining: `patterns.{png,json}` (rasterize AreaFill symbol tiles), the in-bake complex-line period table (`bake.buildLinestyleTable` still S-52 — affects baked complex-line tile geometry), and threading the portrayer through the **bake** CLI / `baker.go` entry points (done at the flip).
   - d. **Bake a real S-64 cell and `magick compare` vs the `s52-portrayal-baseline` tag**; iterate gaps (text labels need the full `PortrayalMain` wrapper).
10. **Embed + decommission** — vendor the catalogue (licence-cleared) under a `go:embed` package, swap `os.DirFS`→`embed.FS`; delete `pkg/s52` (parser, lookup, 24 `cs_*.go`, `preslib`).

## Open gate
- **Licence**: embedding (decided) requires committing ~5–6 MB of the IHO S-101 DRAFT catalogue into the repo. Confirm the catalogue's licence permits redistribution before vendoring/committing. Tag `s52-portrayal-baseline` (commit `f363cea`) preserves the S-52 renderer for `magick compare` regardless.
