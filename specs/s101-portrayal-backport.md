...
9. **Baker surgery** (the remaining core work, replace-then-delete; **decided: embed catalogue, no `--portrayal` flag**):
   - a. Thread an `S101Builder` (engine + drawing `Catalog`) through `internal/engine/baker/baker.go` in place of the 3 `s52.LoadLibraryFromBytes(preslib.DAI)` sites; source the catalogue from a dir now (`os.DirFS`), swap to `embed.FS` once vendored.
   - b. Swap the portrayal seam `bake.go:592` (`portrayal.BuildFeaturePasses`) to `S101Builder.Build` (single pass to start; reinstate plain/symbolized + simplified/paper via S-101 context params + the multi-pass logic later).
   - c. Swap `assets/emit.go` (`ColorTablesJSON`/`LinestylesJSON`/`SpriteAtlas`) to the S-101 catalogue sources (`SpriteAtlasS101FS`, `catalog` colours/linestyles) and `cmd/main.go`.
