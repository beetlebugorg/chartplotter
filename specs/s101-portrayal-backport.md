...
9. **Baker surgery** (replace-then-delete; **decided: embed catalogue, no `--portrayal` flag**):
   ...
   - c. Asset emit — **partly DONE**. `assets.EmitS101`/`EmitS101FS`: colortables.json (from `colorProfile.xml`, byte-identical to S-52) + sprite.{png,json} (724 S-101 symbols). Remaining: `linestyles.json` + `patterns.{png,json}` (format-matching emitters from `catalog` LineStyles/AreaFills), and the in-bake complex-line period table (`bake.buildLinestyleTable` still S-52). Then wire `cmd/main.go` to choose the S-101 emit + portrayer.
