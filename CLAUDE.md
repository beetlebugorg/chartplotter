...
`libtile57` engine (Zig, a sibling `../tile57` clone of github.com/beetlebugorg/tile57)
...
- `make build` — build `bin/chartplotter`. CGO build linking `libtile57` (built on
  demand from the sibling `../tile57` checkout with Zig 0.16); the S-101 catalogue
  is embedded in the lib, so there's no separate sync/embed step. Needs the
  `../tile57` sibling clone + Zig.
...
- **CGO is required.** libtile57 is the sole tile/portrayal engine, linked via CGO,
  ...
  `mattn/go-sqlite3`). The sibling `../tile57` clone (with its nested IHO catalogue
  submodules) + Zig 0.16 are hard build deps.
- **Engine dev override.** The default engine source is the sibling `../tile57`
  checkout — go.mod already replaces the binding at `../tile57/bindings/go` and the
  Makefile defaults `TILE57 ?= ../tile57`, so a normal sibling clone needs no
  `go.work`. `go.work` is OPTIONAL: use it only to point at a DIFFERENT engine
  checkout, overriding BOTH halves — a gitignored `go.work` replacing
